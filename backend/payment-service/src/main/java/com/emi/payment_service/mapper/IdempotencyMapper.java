...
public class IdempotencyMapper {
	...
	public IdempotencyRecord getEntityWithdraw(RequestWithdrawDto request, UUID idempotencyKey, UUID keycloakId) {
		IdempotencyRecord idempotency = new IdempotencyRecord();
		String hashedRequest = String.valueOf(request.hashCode());
	...
	public void updateIdempWithdraw(IdempotencyRecord idempotency, WithdrawResponseDto response) {
		String responseString ="";
		try {
			 responseString = objectMapper.writeValueAsString(response);
			}catch(JsonProcessingException  ex) {
